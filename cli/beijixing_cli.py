#!/usr/bin/env python3
"""BeiJiXing Agent CLI - Main Command Line Interface
...
try:
    from core.logo import BeiJiXingLogo, LogoConfig, display_splash, display_version_info
except ImportError:
    BeiJiXingLogo = None
...
class BeiJiXingCLI:
    ...
    CONFIG_DIR = os.path.expanduser("~/.beijixing")
    MODULES_DIR = os.path.join(CONFIG_DIR, "modules")
    LOG_FILE = os.path.join(CONFIG_DIR, "beijixing.log")
    ...
    def run_interactive(self):
        """运行交互式对话"""
        print(f"\n{LogoConfig.COLORS['GREEN']}Starting BeiJiXing Agent in interactive mode...{LogoConfig.COLORS['ENDC']}\n")
        
        try:
            while True:
                user_input = input(f"{LogoConfig.COLORS['CYAN']}BeiJiXing>{LogoConfig.COLORS['ENDC']} ").strip()
                ...
                if user_input.lower() in ['exit', 'quit', 'q']:
                    ...
                    print(f"BeiJiXing Agent v{self.VERSION}")
                elif user_input.lower() == 'clear':
                    os.system('cls' if os.name == 'nt' else 'clear')
                    logo = BeiJiXingLogo() if BeiJiXingLogo else None
                    if logo:
                        logo.display_welcome()
                elif user_input:
                    print(f"{LogoConfig.COLORS['BLUE']}Processing: {user_input}{LogoConfig.COLORS['ENDC']}")
                    print(f"{LogoConfig.COLORS['GREEN']}This is a demo response.{LogoConfig.COLORS['ENDC']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog='beijixing',
        description='BeiJiXing Agent - Your Intelligent AI Assistant',
...
Quick Commands:
  ...
  %(prog)s disable <module>   Disable a module
    ...
    # 显示启动画面并进入交互模式
    if BeiJiXingLogo:
        logo = BeiJiXingLogo(animate=True)
        logo.display()
